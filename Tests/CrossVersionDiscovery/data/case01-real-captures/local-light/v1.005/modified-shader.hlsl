...
FDeferredLightData GetDeferredLight()
{
    ...
    float3 SampleLightProfile(FResolvedPixel pixel, FDeferredLightData light)
    {
        ...
        float2 profileUV = float2(
            frac(atan2(profileDirection.y, profileDirection.x) * INV_2PI + 1.0),
            asin(profileDirection.z) * INV_PI + 0.5
        );

        return DeferredLightUniforms_SourceTexture.Sample(
            View_SharedBilinearWrappedSampler, profileUV).rgb;
    }
#endif
    ...
    for (int i = 0; i < CONTACT_SHADOWS_SAMPLES; ++i)
    {
        ...
        float penetration = rayDepth - sceneDepth;
			...
			if (penetration > contactShadowBias && penetration < thickness)
			{
				...
				float thicknessFade = 1.0 - saturate((penetration - contactShadowBias) / (thickness - contactShadowBias));
