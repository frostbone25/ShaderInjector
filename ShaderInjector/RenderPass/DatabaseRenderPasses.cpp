...
namespace DatabaseRenderPasses
{
	...
	void RefreshRenderPasses()
	{
		...
		for (const std::string& jsonPath : jsonPaths)
		{
			...
			const bool storedAtRenderPassRoot = ShaderInjectorIO::PathsEqual(containingDirectory, renderPassDirectory);
			const std::string expectedPackageJsonName = ShaderInjectorIO::FileNameFromPath(containingDirectory) + ShaderInjectorIO::extensionJSON;
			...
			{
				ShaderInjectorIO::WriteToLogFileWarning("DatabaseRenderPasses->RefreshRenderPasses: ignoring invalid render pass " + jsonPath);
				continue;
			}
			...
			if (duplicateId)
			{
				ShaderInjectorIO::WriteToLogFileWarning("DatabaseRenderPasses->RefreshRenderPasses: duplicate id " + renderPass.id);
				continue;
			}

			const bool legacyRootDocument = ShaderInjectorIO::PathsEqual(renderPass.packageDirectory, renderPassDirectory);

			if (legacyRootDocument && !RenderPassPackageUsesCurrentName(renderPass) &&
				!MoveRenderPassPackageToCurrentName(renderPass))
			{
				ShaderInjectorIO::WriteToLogFileWarning("DatabaseRenderPasses->RefreshRenderPasses: could not normalize package path for " + renderPass.name);
			}
		...
		ShaderInjectorIO::WriteToLogFile("DatabaseRenderPasses->RefreshRenderPasses: loaded render passes=" + std::to_string(gRenderPasses.size()));
	}
	...
	bool IsEventChainActive(const RenderPass::RenderPassDisk& renderPass)
	{
		...
		const bool mipChainPass = renderPass.type == RenderPass::RenderPassType::MipChain && !RenderPass::FindMipChainRuntimeSource(renderPass);
		const RenderPass::RenderPassDisk* currentRenderPass = &renderPass;
		std::unordered_set<std::string> visitedRenderPassIds;
	...
	bool CreateRenderPass(std::string& outRenderPassId)
	{
		...
		const std::string fileStem = ShaderInjectorIO::SanitizeFileStem(renderPass.name);
		renderPass.packageDirectory = ShaderInjectorIO::JoinPath(ShaderInjectorIO::GetRenderPassesDirectory(), fileStem);
		renderPass.jsonPath = ShaderInjectorIO::JoinPath(renderPass.packageDirectory, fileStem + ShaderInjectorIO::extensionJSON);
	...
	bool SaveRenderPass(const std::string& renderPassId)
	{
		...
		for (const RenderPass::SamplerStateDisk& sampler : renderPass->samplers)
		{
			...
			const std::string bindingKey = std::to_string(sampler.shaderRegister) + ':' + std::to_string(sampler.registerSpace);
