...
//SamplerComparisonState View_SharedTrilinearWrappedSampler : register(s3, space0); // can't disambiguate
//SamplerComparisonState View_SharedTrilinearClampedSampler : register(s4, space0); // can't disambiguate
//SamplerComparisonState View_SharedAnisotropic4XClampedSampler : register(s5, space0); // can't disambigua

SamplerState View_SharedPointClampedSampler : register(s0, space0); // can't disambiguate
SamplerState View_SharedBilinearWrappedSampler : register(s1, space0); // can't disambiguate
SamplerState View_SharedBilinearClampedSampler : register(s2, space0); // can't disambiguate
SamplerState View_SharedTrilinearWrappedSampler : register(s3, space0); // can't disambiguate
SamplerState View_SharedTrilinearClampedSampler : register(s4, space0); // can't disambiguate
SamplerState View_SharedAnisotropic4XClampedSampler : register(s5, space0); // can't disambiguatete

Texture3D<float4> View_SpatiotemporalBlueNoiseVolumeTexture : register(t0, space0);
Texture3D<float4> View_PerlinNoiseVolumeTexture : register(t1, space0);

#if defined(WATER_SHADER_1_0_0_4_VERSION)
    ...
    Texture2D<float4> TranslucentBasePass_SceneColorCopyTexture : register(t12, space0);
    Texture2D<float4> Material_Texture2D_13 : register(t13, space0);
    Texture2D<float4> Material_Texture2D_14 : register(t14, space0);
    Texture2D<float4> Material_Texture2D_15 : register(t15, space0);
    Texture2D<float4> Material_Texture2D_16 : register(t16, space0);
    Texture2D<float4> Material_Texture2D_17 : register(t17, space0);
    Texture2D<float4> Material_Texture2D_19 : register(t18, space0);
    ...
    Texture2D<float4> TranslucentBasePass_SceneColorCopyTexture : register(t11, space0);
    Texture2D<float4> Material_Texture2D_13 : register(t12, space0);
    Texture2D<float4> Material_Texture2D_14 : register(t13, space0);
    Texture2D<float4> Material_Texture2D_15 : register(t14, space0);
    Texture2D<float4> Material_Texture2D_16 : register(t15, space0);
    Texture2D<float4> Material_Texture2D_17 : register(t16, space0);
    Texture2D<float4> Material_Texture2D_19 : register(t17, space0);
    Texture2D<float4> ForwardLightProfileTextures[] : register(t0, space13);
...
cbuffer Primitive : register(b1, space0) 
{
	row_major float4x4 Primitive_LocalToWorld : packoffset(c0.x);
	float4 Primitive_InvNonUniformScaleAndDeterminantSign : packoffset(c4.x);
	float4 Primitive_ObjectWorldPositionAndRadius : packoffset(c5.x);
	row_major float4x4 Primitive_WorldToLocal : packoffset(c6.x);
	row_major float4x4 Primitive_PreviousLocalToWorld : packoffset(c10.x);
	row_major float4x4 Primitive_PreviousWorldToLocal : packoffset(c14.x);
	float3 Primitive_ActorWorldPosition : packoffset(c18.x);
	float Primitive_UseSingleSampleShadowFromStationaryLights : packoffset(c18.w);
	float3 Primitive_ObjectBounds : packoffset(c19.x);
	float Primitive_LpvBiasMultiplier : packoffset(c19.w);
	float Primitive_DecalReceiverMask : packoffset(c20.x);
	float Primitive_PerObjectGBufferData : packoffset(c20.y);
	float Primitive_UseVolumetricLightmapShadowFromStationaryLights : packoffset(c20.z);
	float Primitive_DrawsVelocity : packoffset(c20.w);
	float4 Primitive_ObjectOrientation : packoffset(c21.x);
	float4 Primitive_NonUniformScale : packoffset(c22.x);
	float3 Primitive_LocalObjectBoundsMin : packoffset(c23.x);
	int Primitive_LightingChannelMask : packoffset(c23.w);
	float3 Primitive_LocalObjectBoundsMax : packoffset(c24.x);
	int Primitive_LightmapDataIndex : packoffset(c24.w);
	float3 Primitive_PreSkinnedLocalBoundsMin : packoffset(c25.x);
	int Primitive_SingleCaptureIndex : packoffset(c25.w);
	float3 Primitive_PreSkinnedLocalBoundsMax : packoffset(c26.x);
	int Primitive_OutputVelocity : packoffset(c26.w);
	int Primitive_Reserved0 : packoffset(c27.x);
	int Primitive_Reserved1 : packoffset(c27.y);
	int Primitive_PixelAttribute : packoffset(c27.z);
	float Primitive_DynamicCoverage : packoffset(c27.w);
};

cbuffer TranslucentBasePass : register(b2, space0) 
{
...
cbuffer ReflectionCapture : register(b3, space0) 
{
...
cbuffer Material : register(b4, space0) 
{
...
struct InputStruct
{
    ...
    float4 VERTEX_EXPRESSION : VERTEX_EXPRESSION;
    float4 ENVIRONMENT_AMBIENT_DICE_0 : ENVIRONMENT_AMBIENT_DICE_0;
    float4 ENVIRONMENT_AMBIENT_DICE_1 : ENVIRONMENT_AMBIENT_DICE_1;
    float4 ENVIRONMENT_AMBIENT_DICE_2 : ENVIRONMENT_AMBIENT_DICE_2;
...
{
    // The DXIL only consumes xyz from these register rows.
    float3 position =
    ...
    float4 blueNoise)
{
    ...
    // Match the original truncation exactly. Shadow atlas coordinates are
    // positive, so this is normally equivalent to floor.
...
{
    // Keep the packed-buffer and BRDF arithmetic bit-for-bit recognizable while
    // containing the decompiler's register-style temporaries inside this stage.
    ...
    float randomOffset)
{
    ...
    // Project the shaded point and the reflection-ray endpoint to clip space.
    // The nested mad order mirrors the original DXIL matrix expansion.
    ...
    if (reflectionHitConfidence > 0.0)
    {
        ...
        // Fold coordinates outside [-1,1] back toward the viewport edge.
...
int2 RefractionUvToTexel(float2 uv)
{
...
bool RefractionTexelIsBelowWater(int2 texel, float waterSurfaceWorldZ)
{
    ...
    return receiverWorldPosition.z <= waterSurfaceWorldZ + REFRACTION_WATER_PLANE_Z_BIAS;
}

bool RefractionUvFootprintIsBelowWater(float2 uv, float waterSurfaceWorldZ)
{
    uv = ClampRefractionUvToView(uv);
    ...
    // The candidate is accepted only when every texel touched by bilinear
    // filtering belongs below the water surface.
    return
        RefractionTexelIsBelowWater(texel00, waterSurfaceWorldZ) &&
        RefractionTexelIsBelowWater(texel10, waterSurfaceWorldZ) &&
        RefractionTexelIsBelowWater(texel01, waterSurfaceWorldZ) &&
        RefractionTexelIsBelowWater(texel11, waterSurfaceWorldZ);
}

bool RefractionCandidateIsSafe(
    float2 refractedUv,
    float2 unrefractedUv,
    float waterSurfaceWorldZ)
{
    bool isSafe = RefractionUvFootprintIsBelowWater(refractedUv, waterSurfaceWorldZ);

#if defined(ENABLE_REFRACTION_CHROMATIC_DISPERSION)
    // Include the outer chromatic footprints in the same all-or-nothing test.
    float2 dispersionOffset = (refractedUv - unrefractedUv) * REFRACTION_DISPERSION_STRENGTH;
    isSafe =
        isSafe &&
        RefractionUvFootprintIsBelowWater(refractedUv + dispersionOffset, waterSurfaceWorldZ) &&
        RefractionUvFootprintIsBelowWater(refractedUv - dispersionOffset, waterSurfaceWorldZ);
#endif
...
WaterSurfaceEvaluation EvaluateWaterSurface(InputStruct _IN)
{
    ...
    bool frontFacingSurface;
    float sceneIntersectionFade;
    float2 pixelXY = _IN.SV_Position.xy;

    // Material inputs, noise and world-position reconstruction
    float proceduralDensity;

    surface.MaterialUV = _IN.TEXCOORD0.xy;
    ...
    int3 blueNoiseTexel = int3(int2(pixelXY) & 127, ((int)View_StateFrameIndex) & 7);
    float4 blueNoiseSample = View_SpatiotemporalBlueNoiseVolumeTexture.Load(int4(blueNoiseTexel, 0));

    float materialPhase = Material_VectorExpressions[15].x;
    float4 materialSample13 = Material_Texture2D_13.SampleBias(View_SharedAnisotropic4XClampedSampler, surface.MaterialUV, View_TemporalSamplerBias.y, int2(0, 0));
    float4 materialSample14 = Material_Texture2D_14.SampleBias(View_SharedAnisotropic4XClampedSampler, surface.MaterialUV, View_TemporalSamplerBias.y, int2(0, 0));
    float4 materialSample15 = Material_Texture2D_15.SampleBias(View_SharedAnisotropic4XClampedSampler, surface.MaterialUV, View_TemporalSamplerBias.y, int2(0, 0));
    float4 materialSample16 = Material_Texture2D_16.SampleBias(View_SharedAnisotropic4XClampedSampler, surface.MaterialUV, View_TemporalSamplerBias.y, int2(0, 0));
    float4 materialSample17 = Material_Texture2D_17.SampleBias(View_SharedAnisotropic4XClampedSampler, surface.MaterialUV, View_TemporalSamplerBias.y, int2(0, 0));
    float spectralExponentScale = 1.00000 / (max(1.00000e-06, ((materialSample17[0]) * 100.00000)));
    float greenExtinction = exp2(((log2((saturate((materialSample16[1]))))) * spectralExponentScale));
    float4 materialSample19 = Material_Texture2D_19.SampleLevel(View_SharedBilinearClampedSampler, surface.MaterialUV, 0.000000, int2(0, 0));
    float materialCombinedValue = surface.WorldPosition.x - _IN.VERTEX_EXPRESSION.x;

    // Six-octave wrapped value-noise density (fBm-style accumulation).
    float amplitude = 2.5;
    float frequency = 1.0;
    float densityAccumulation = 0.0;
    float2 densityPosition = (surface.TranslatedWorldPosition.xy - _IN.VERTEX_EXPRESSION.xy) * 0.0025;
    float2 densityTranslation = View_PreViewTranslation.xy * -0.0025;

    [unroll]
    for (int octave = 0; octave < 6; ++octave)
    {
        float2 scaledPosition = densityPosition * frequency;
        float2 scaledTranslation = densityTranslation * frequency;
        float2 noisePosition = frac(frac(scaledTranslation * 0.01) + frac(scaledPosition * 0.01)) * 256.0;
        float valueNoise = 0.5 * WrappedValueNoise2D(noisePosition);
        float phaseJitter = frac(materialPhase * 7.95775 * sqrt(frequency * 0.00153938));
        float2 phase = frac(frac(scaledTranslation) + frac(scaledPosition) + phaseJitter + valueNoise) * 6.28319;

        float2 ridge = 1.0 - abs(sin(phase));
        float2 roundedRidge = (abs(cos(phase)) - ridge) * ridge + ridge;
        float octaveDensity = 1.0 - sqrt(roundedRidge.x * roundedRidge.y);
        proceduralDensity = mad(octaveDensity, amplitude, densityAccumulation);

        densityPosition = RotateDensityOctave(densityPosition);
        densityTranslation = RotateDensityOctave(densityTranslation);
        frequency *= 2.0;
        amplitude *= 0.5;
        densityAccumulation = proceduralDensity;
    }

    // Third axis of the interpolated tangent frame.
    float3 tangentFrameThirdAxis = -cross(_IN.TEXCOORD10.xyz, _IN.TEXCOORD11.xyz) * _IN.TEXCOORD11.w;
    float surfaceTerm0 = exp2(((log2((saturate((materialSample14[0]))))) * (1.00000 / (max(1.00000e-06, ((materialSample15[0]) * 100.00000))))));
    float surfaceTerm1 = exp2(((log2((saturate((materialSample16[0]))))) * spectralExponentScale));
    float surfaceTerm2 = exp2(((log2((saturate((materialSample16[2]))))) * spectralExponentScale));
    float surfaceTerm3 = surface.WorldPosition.y - _IN.VERTEX_EXPRESSION.y;
    float4 surfaceSample0 = View_PerlinNoiseVolumeTexture.Sample(View_SharedTrilinearWrappedSampler, float3((materialCombinedValue * 0.00390625), (surfaceTerm3 * 0.00390625), (materialPhase * 0.0625000)), int3(0, 0, 0));
    float4 surfaceSample1 = View_PerlinNoiseVolumeTexture.Sample(View_SharedTrilinearWrappedSampler, float3((materialPhase * 0.125000), (materialCombinedValue * 0.00195313), (surfaceTerm3 * 0.00195313)), int3(0, 0, 0));
    float surfaceTerm4 = surface.WorldPosition.z - ((materialSample19[2]) * 100.00000);
    float surfaceTerm5 = surfaceTerm4 * 0.00250000;
    float surfaceTerm6 = 1.00000 - surfaceTerm5;
    float surfaceTerm7 = ((saturate((1.00000 - (surfaceTerm4 * 0.00333333)))) * (saturate(surfaceTerm6))) * (saturate((1.00000 - ((sqrt(((materialSample19.x * materialSample19.x) + (materialSample19.y * materialSample19.y)))) * 0.00166667))));
    float surfaceTerm8 = surfaceTerm7 * surfaceTerm7;
    float surfaceTerm9 = saturate((((_IN.VERTEX_EXPRESSION.z) - _IN.VERTEX_EXPRESSION.w) * 0.0333333));
    float surfaceTerm10 = saturate(((abs(_IN.VERTEX_EXPRESSION.w)) * 0.0166667));
    float surfaceTerm11 = ((saturate((proceduralDensity * 0.100000))) * 0.0500000) + 0.950000;
    float surfaceTerm12 = surfaceTerm11 * surfaceTerm11;
    float surfaceTerm13 = surfaceTerm12 * surfaceTerm12;
    float surfaceTerm14 = saturate(((surfaceTerm13 * surfaceTerm13) * (((sqrt(((surfaceSample1[0]) * (surfaceSample0[0])))) * 0.500000) + 0.500000)));
    bool surfaceTest0 = ((surfaceTerm14 + surfaceTerm8) > 1.00000);
    float surfaceTerm15 = saturate((((sqrt((saturate((((surfaceTerm8 + -1.00000) + surfaceTerm14) * surfaceTerm14))))) * (((surfaceTerm9 * surfaceTerm9) * (1.00000 - surfaceTerm10)) + surfaceTerm10)) * ((float)(uint)(surfaceTest0))));
    float surfaceTerm16 = (surfaceTerm15 * surfaceTerm15) * (3.00000 - (surfaceTerm15 * 2.00000));
    float surfaceTerm17 = surfaceTerm16 * surfaceTerm16;
    float surfaceCoverage = saturate((surfaceTerm17 * surfaceTerm17));
    float surfaceTerm18 = (((sqrt(surfaceTerm8)) * ((surfaceTerm14 * 4.00000) - proceduralDensity)) + proceduralDensity) + (surfaceTerm16 * 6.00000);

    // Reconstruct a procedural displaced normal from screen derivatives.
    float3 basePositionDx = ddx_fine(surface.TranslatedWorldPosition);
    float3 basePositionDy = ddy_fine(surface.TranslatedWorldPosition);
    float3 baseGeometricNormal = SafeNormalize(cross(basePositionDx, basePositionDy));
    float3 displacedPosition = surface.TranslatedWorldPosition + baseGeometricNormal * surfaceTerm18;
    float3 displacedPositionDx = ddx_fine(displacedPosition);
    float3 displacedPositionDy = ddy_fine(displacedPosition);
    float3 displacedGeometricNormal = SafeNormalize(cross(displacedPositionDx, displacedPositionDy));

    float3 correctedNormal = SafeNormalize(_IN.TEXCOORD11.xyz - baseGeometricNormal + displacedGeometricNormal);
    float normalFadeDistance = _IN.SV_Position.w * 1.0e-5;
    float normalFade = rcp(normalFadeDistance * normalFadeDistance + 1.0);
    float3 tangentNormal = lerp(
        float3(_IN.TEXCOORD10.z, tangentFrameThirdAxis.z, _IN.TEXCOORD11.z),
        float3(dot(_IN.TEXCOORD10.xyz, correctedNormal), dot(tangentFrameThirdAxis, correctedNormal), dot(_IN.TEXCOORD11.xyz, correctedNormal)),
        normalFade);

    tangentNormal = SafeNormalize(tangentNormal);
    surface.Normal = SafeNormalize(_IN.TEXCOORD10.xyz * tangentNormal.x + tangentFrameThirdAxis * tangentNormal.y + _IN.TEXCOORD11.xyz * tangentNormal.z);
    float4 surfacePrimitiveReserved0Data = float4(Primitive_Reserved0, Primitive_Reserved1, Primitive_PixelAttribute, Primitive_DynamicCoverage);
    bool surfaceTest1 = ((surfacePrimitiveReserved0Data[3]) > (((blueNoiseSample[3]) * 0.996094) + 0.00195313));

    if (surfaceTest1)
    {
        bool surfaceTest2 = (((blueNoiseSample.x * 0.996094) + 0.00195313) < 1.00000);
        frontFacingSurface = surfaceTest2;
    }
    else
    {
        frontFacingSurface = false;
    }

    float surfaceSelectedValue = frontFacingSurface ? 1.00000 : -1.00000;
    bool surfaceTest3 = (surfaceSelectedValue < 0.000000);

    if (surfaceTest3)
        discard;

    // Toksvig-style normal-variance roughness filtering.
    float normalCoverage = saturate(sqrt(surfaceCoverage));
    ...
    surface.FilteredRoughness = sqrt(saturate(min(normalVariance * 0.159155, 0.18) + normalCoverage * normalCoverage));
    ...
    int primitiveLightingFlags = (int)Primitive_PixelAttribute;
    bool surfaceTest4 = (primitiveLightingFlags & 2) == 0;

    // Reflection and refraction directions. 0.750188 is eta; 0.562781 is eta^2.
    float3 viewDirection = -surface.CameraToPixelDirection;
    float normalViewDot = dot(surface.Normal, viewDirection);
    float surfaceAbsoluteValue = abs(normalViewDot);
    float3 reflectionBasis = normalViewDot * surface.Normal + surface.CameraToPixelDirection;
    //float3 reflectedDirection = SafeNormalize(2.0 * reflectionBasis + surfaceAbsoluteValue * surface.Normal);
    ...
    float3 refractedViewDirection = SafeNormalize(0.750188 * reflectionBasis - transmittedCosine * surface.Normal);
    ...
    //surface.ReflectionVector = lerp(reflectedDirection, surface.Normal, surface.RoughnessCubed);
    surface.ReflectionVector =reflectedDirection;
    surface.ReflectionDirection = SafeNormalize(surface.ReflectionVector);
    surface.TransmissionDirection = SafeNormalize(lerp(refractedViewDirection, -surface.Normal, surface.RoughnessCubed));
    float surfaceTerm19 = max(1.00000e-05, surfaceAbsoluteValue);
    float surfaceTerm20 = View_BufferSizeAndInvSize.z * pixelXY.x;
    float surfaceTerm21 = View_BufferSizeAndInvSize.w * pixelXY.y;
    // Seed depth only scales the wave-normal refraction offset. Receiver fog
    // and extinction are evaluated later from the final unified UV.
    int surfaceIndex0 = asint((uint)(pixelXY.x));
    int surfaceIndex1 = asint((uint)(pixelXY.y));
    float4 surfaceSample2 = TranslucentBasePass_SceneTextures_SceneDepthTexture.Load(int3(surfaceIndex0, surfaceIndex1, 0));
    float surfaceTerm22 = ((View_InvDeviceZToWorldZTransform.x * surfaceSample2.x) + View_InvDeviceZToWorldZTransform.y) + (1.00000 / ((View_InvDeviceZToWorldZTransform.z * surfaceSample2.x) - View_InvDeviceZToWorldZTransform.w));
    float surfaceTerm23 = surfaceTerm22 - _IN.SV_Position.w;
    float surfaceTerm24 = max(0.000000, surfaceTerm23);
    float surfaceTerm25 = ((float)asuint(surfaceIndex0)) + 0.500000;
    float surfaceTerm26 = ((float)asuint(surfaceIndex1)) + 0.500000;
    float4 surfaceTerm27 = View_SVPositionToTranslatedWorld[0];
    float4 surfaceTerm28 = View_SVPositionToTranslatedWorld[1];
    float4 surfaceTerm29 = View_SVPositionToTranslatedWorld[2];
    float4 surfaceTerm30 = View_SVPositionToTranslatedWorld[3];
    float4 surfaceViewPreViewTranslationData = float4(View_PreViewTranslation.x, View_PreViewTranslation.y, View_PreViewTranslation.z, PrePadding_View_1260);
    float surfaceTerm31 = (((mad(surfaceSample2.x, surfaceTerm29.z, (mad(surfaceTerm26, surfaceTerm28.z, (surfaceTerm27.z * surfaceTerm25))))) + surfaceTerm30.z) / ((mad(surfaceSample2.x, surfaceTerm29.w, (mad(surfaceTerm26, surfaceTerm28.w, (surfaceTerm27.w * surfaceTerm25))))) + surfaceTerm30.w)) - surfaceViewPreViewTranslationData.z;
    float surfaceTerm32 = (saturate(((1.00000 / (surfaceTerm24 + 49.0000)) * surfaceTerm24))) * 50.0000;
    float surfaceTerm33 = mad(refractedViewDirection.z, (View_TranslatedWorldToView[2][0]), (mad(refractedViewDirection.y, (View_TranslatedWorldToView[1][0]), (refractedViewDirection.x * (View_TranslatedWorldToView[0][0])))));
    float surfaceTerm34 = mad(refractedViewDirection.z, (View_TranslatedWorldToView[2][1]), (mad(refractedViewDirection.y, (View_TranslatedWorldToView[1][1]), (refractedViewDirection.x * (View_TranslatedWorldToView[0][1])))));
    float surfaceTerm35 = mad(refractedViewDirection.z, (View_TranslatedWorldToView[2][2]), (mad(refractedViewDirection.y, (View_TranslatedWorldToView[1][2]), (refractedViewDirection.x * (View_TranslatedWorldToView[0][2])))));
    float surfaceTerm36 = dot(float3(surfaceTerm33, surfaceTerm34, surfaceTerm35), float3(surfaceTerm33, surfaceTerm34, surfaceTerm35));
    float surfaceInverseLength = rsqrt(surfaceTerm36);
    float surfaceTerm37 = max(10.0000, _IN.SV_Position.w);
    float4 surfaceTerm38 = View_ClipToView[0];
    float4 surfaceTerm39 = View_ClipToView[1];
    float surfaceTerm40 = (((surfaceTerm33 * surfaceTerm32) * surfaceInverseLength) * (1.00000 / ((surfaceTerm38[0]) * surfaceTerm37))) * (View_ScreenPositionScaleBias[0]);
    float surfaceTerm41 = (((surfaceTerm34 * surfaceTerm32) * surfaceInverseLength) * (1.00000 / ((surfaceTerm39[1]) * surfaceTerm37))) * (View_ScreenPositionScaleBias[1]);
    float surfaceTerm42 = ((View_BufferSizeAndInvSize.x * surfaceTerm20) - View_ViewRectMin.x) * View_ViewSizeAndInvSize.z;
    float surfaceTerm43 = ((View_BufferSizeAndInvSize.y * surfaceTerm21) - View_ViewRectMin.y) * View_ViewSizeAndInvSize.w;
    float surfaceTerm44 = min(View_ViewSizeAndInvSize.z, View_ViewSizeAndInvSize.w);
    float surfaceTerm45 = surfaceTerm44 * View_ViewSizeAndInvSize.y;
    float surfaceTerm46 = surfaceTerm44 * View_ViewSizeAndInvSize.x;
    float surfaceTerm47 = (12.5000 / surfaceTerm45) * (abs(surfaceTerm40));
    float surfaceTerm48 = (12.5000 / surfaceTerm46) * (abs(surfaceTerm41));
    bool surfaceTest5 = (surfaceTerm40 > 0.000000);
    bool surfaceTest6 = (surfaceTerm41 > 0.000000);
    bool surfaceTest7 = (surfaceTerm40 < 0.000000);
    bool surfaceTest8 = (surfaceTerm41 < 0.000000);
    int surfaceIndex2 = (int)(surfaceTest5);
    int surfaceIndex3 = (int)(surfaceTest6);
    int surfaceIndex4 = (int)(surfaceTest7);
    int surfaceIndex5 = (int)(surfaceTest8);
    int surfaceIndex6 = surfaceIndex2 - surfaceIndex4;
    int surfaceIndex7 = surfaceIndex3 - surfaceIndex5;
    float surfaceTerm49 = (((((float)(surfaceIndex6)) * 0.0800000) * surfaceTerm45) * surfaceTerm47) * (1.00000 / (surfaceTerm47 + 1.00000));
    float surfaceTerm50 = (((((float)(surfaceIndex7)) * 0.0800000) * surfaceTerm46) * surfaceTerm48) * (1.00000 / (surfaceTerm48 + 1.00000));
    float surfaceTerm51 = saturate((abs(((surfaceTerm42 * 2.00000) + -1.00000))));
    float surfaceTerm52 = saturate((abs(((surfaceTerm43 * 2.00000) + -1.00000))));
    float surfaceTerm53 = surfaceTerm51 * surfaceTerm51;
    float surfaceTerm54 = surfaceTerm52 * surfaceTerm52;
    float surfaceTerm55 = (View_ViewRectMin.x + -1.00000) + View_ViewSizeAndInvSize.x;
    float surfaceTerm56 = (View_ViewRectMin.y + -1.00000) + View_ViewSizeAndInvSize.y;
    int surfaceIndex8 = (int)((min((max(((View_ViewSizeAndInvSize.x * ((surfaceTerm49 + surfaceTerm42) - ((surfaceTerm53 * surfaceTerm53) * surfaceTerm49))) + View_ViewRectMin.x), View_ViewRectMin.x)), surfaceTerm55)));
    int surfaceIndex9 = (int)((min((max(((View_ViewSizeAndInvSize.y * ((surfaceTerm50 + surfaceTerm43) - ((surfaceTerm54 * surfaceTerm54) * surfaceTerm50))) + View_ViewRectMin.y), View_ViewRectMin.y)), surfaceTerm56)));
    float surfaceTerm57 = ((float)(surfaceIndex8)) + 0.500000;
    float surfaceTerm58 = ((float)(surfaceIndex9)) + 0.500000;
    // The unrefracted receiver depth is used only to retain the existing
    // distance-dependent refraction strength. It no longer drives fog.
    float refractionOffsetFade = saturate(surfaceTerm24 * 0.02);
    float surfaceTerm67 = 1.00000 - surface.FilteredRoughness;
    float surfaceTerm68 = (((min((surfaceTerm67 * surfaceTerm67), (exp2((surfaceTerm19 * -9.28000))))) * surfaceTerm67) + (0.0425000 - (surface.FilteredRoughness * 0.0275000))) * 1.04000;
    float surfaceTerm69 = 1.00000 - (saturate(((((surface.FilteredRoughness * 0.0220000) + -0.0400000) + surfaceTerm68) + (((1.04000 - (surface.FilteredRoughness * 0.572000)) - surfaceTerm68) * 0.0203732))));
    float surfaceTerm70 = 0.0;
    float surfaceTerm72 = log2((max(0.000000, surfaceTerm0)));
    float surfaceTerm73 = 1.0;
    float surfaceTerm74 = log2((max(0.000000, surfaceTerm1)));
    float surfaceTerm75 = log2((max(0.000000, greenExtinction)));
    float surfaceTerm76 = log2((max(0.000000, surfaceTerm2)));
    float surfaceTerm77 = 1.0;
    float surfaceTerm78 = 1.0;
    float surfaceTerm79 = 1.0;
    float surfaceTerm81 = 1.0;
    float surfaceTerm82 = 0.0;
    float surfaceTerm83 = 0.0;
    float surfaceTerm84 = 0.0;
    float surfaceTransmittanceMask = 1.00000 - surfaceCoverage;
    float surfaceTerm85 = max(0.0400000, surface.FilteredRoughness);
    float surfaceTerm86 = max(1.00000e-06, (1.00000 - (saturate(((((surfaceTerm85 * surfaceTerm19) + 0.0266916) / (surfaceTerm19 + 0.466495)) * (exp2((((1.00000 / surfaceTerm85) * surfaceTerm19) * (log2(surfaceTerm85))))))))));
    float surfaceTerm87 = 1.00000 - surfaceTerm19;
    float surfaceTerm88 = surfaceTerm87 * surfaceTerm87;
    float surfaceTerm89 = ((surfaceTerm88 * surfaceTerm88) * surfaceTerm87) * (exp2(((log2((((exp2(((surfaceTerm85 * 4.77030) * (log2(surfaceTerm19))))) * 2.36651) + 0.0387332))) * surfaceTerm85)));
    float surfaceTerm90 = ((((1.00000 / surfaceTerm86) * (1.00000 - surfaceTerm86)) * dielectricF0) + 1.00000) * (((surfaceTerm86 - surfaceTerm89) * dielectricF0) + surfaceTerm89);
    float surfaceTerm91 = dot(float3(surfaceTerm90, surfaceTerm90, surfaceTerm90), float3(0.333333, 0.333333, 0.333333));
    float multipleScatteringWeight = saturate(surfaceTerm90);
    float transmissionWeight = saturate((exp2(((log2((surfaceTerm19 + 1.00000))) * (exp2((-1.00000 - (surface.FilteredRoughness * 16.0000))))))));
    int surfaceIndex12 = asint((uint)((((View_LightProbeSizeRatioAndInvSizeRatio[2]) * pixelXY.x) - (View_ViewRectMin[0]))));
    int surfaceIndex13 = asint((uint)((((View_LightProbeSizeRatioAndInvSizeRatio[3]) * pixelXY.y) - (View_ViewRectMin[1]))));
    int4 surfaceTranslucentbasepassShared_Forward_LightGridPixelSizeShiftData = float4(TranslucentBasePass_Shared_Forward_LightGridPixelSizeShift, PrePadding_TranslucentBasePass_Shared_Forward_36, PrePadding_TranslucentBasePass_Shared_Forward_40, PrePadding_TranslucentBasePass_Shared_Forward_44);
    float4 surfaceTranslucentbasepassShared_Forward_LightGridZParamsData = float4(TranslucentBasePass_Shared_Forward_LightGridZParams.x, TranslucentBasePass_Shared_Forward_LightGridZParams.y, TranslucentBasePass_Shared_Forward_LightGridZParams.z, PrePadding_TranslucentBasePass_Shared_Forward_60);
    int4 surfaceTranslucentbasepassShared_Forward_CulledGridSizeData = float4(TranslucentBasePass_Shared_Forward_CulledGridSize.x, TranslucentBasePass_Shared_Forward_CulledGridSize.y, TranslucentBasePass_Shared_Forward_CulledGridSize.z, TranslucentBasePass_Shared_Forward_MaxCulledLightsPerCell);
    int surfaceIndex14 = asint((uint)((max(0.000000, ((log2((((surfaceTranslucentbasepassShared_Forward_LightGridZParamsData[0]) * _IN.SV_Position.w) + (surfaceTranslucentbasepassShared_Forward_LightGridZParamsData[1])))) * (surfaceTranslucentbasepassShared_Forward_LightGridZParamsData[2]))))));
    int surfaceIndex15 = surfaceTranslucentbasepassShared_Forward_CulledGridSizeData.z + -1;
    int surfaceTerm92 = (int)min((uint)(surfaceIndex14), (uint)(surfaceIndex15));
    int surfaceIndex16 = surfaceTranslucentbasepassShared_Forward_LightGridPixelSizeShiftData.x & 31;
    int surfaceIndex17 = surfaceIndex12 >> surfaceIndex16;
    int surfaceIndex18 = surfaceIndex13 >> surfaceIndex16;
    int surfaceIndex19 = surfaceTerm92 * surfaceTranslucentbasepassShared_Forward_CulledGridSizeData.y;
    int surfaceIndex20 = surfaceIndex19 + surfaceIndex18;
    int surfaceIndex21 = surfaceIndex20 * surfaceTranslucentbasepassShared_Forward_CulledGridSizeData.x;
    int surfaceIndex22 = surfaceIndex21 + surfaceIndex17;
    int4 surfaceTranslucentbasepassShared_Forward_NumLocalLightsData = float4(TranslucentBasePass_Shared_Forward_NumLocalLights, TranslucentBasePass_Shared_Forward_NumReflectionCaptures, TranslucentBasePass_Shared_Forward_HasDirectionalLight, TranslucentBasePass_Shared_Forward_NumGridCells);

    // Build the one receiver UV used by refraction color, water-fog depth,
    // extinction, and projected caustics. The first part preserves the existing
    // wave-normal refraction target and its artistic blur/flow distortion.
    float2 unrefractedUv = float2(surfaceTerm20, surfaceTerm21);
    float2 refractionUv =
        unrefractedUv +
        refractionOffsetFade *
        View_BufferSizeAndInvSize.zw *
        (float2(surfaceTerm57, surfaceTerm58) - pixelXY);
    refractionUv *= lerp(1.0, lerp(1.0, blueNoiseSample.xy, 0.1), saturate(surfaceTerm24 * 0.00025));

//sceneLinearDepth

    float3 refractionNoisePosition = surface.WorldPosition * 0.001 + View_RealTime * 0.1;
    float2 refractionNoise = View_PerlinNoiseVolumeTexture.SampleLevel(View_SharedBilinearWrappedSampler, refractionNoisePosition, 0).rg;
    refractionUv *= lerp(float2(1.0, 1.0), refractionNoise, 0.01);
    refractionUv += refractionNoise * 0.01;

    // Resolve one authoritative UV. If the complete refraction, stochastic,
    // bilinear, and chromatic footprint touches anything above water, every
    // downstream optical term falls back to the untouched native screen UV.
    refractionUv = ClampRefractionUvToView(refractionUv);
    refractionUv = RefractionCandidateIsSafe(refractionUv, unrefractedUv, surface.WorldPosition.z)
        ? refractionUv
        : unrefractedUv;

    int2 refractionReceiverTexel = RefractionUvToTexel(refractionUv);
    float refractionReceiverDeviceDepth = TranslucentBasePass_SceneTextures_SceneDepthTexture.Load(int3(refractionReceiverTexel, 0)).x;
    float refractionReceiverLinearDepth = DeviceZToWorldDepth(refractionReceiverDeviceDepth);
    float2 refractionReceiverPixel = float2(refractionReceiverTexel) + 0.5;
    float3 refractionReceiverWorldPosition = surface.WorldPosition;

    if (refractionReceiverDeviceDepth != 0.0)
    {
        float3 refractionReceiverTranslatedWorldPosition = ReconstructTranslatedWorldPosition(refractionReceiverPixel, refractionReceiverDeviceDepth);
        refractionReceiverWorldPosition = refractionReceiverTranslatedWorldPosition - View_PreViewTranslation.xyz;
    }

    bool receiverDepthIsMissing = refractionReceiverDeviceDepth == 0.0;
    bool hasRefractionReceiver =
        !receiverDepthIsMissing &&
        refractionReceiverLinearDepth > _IN.SV_Position.w &&
        refractionReceiverWorldPosition.z <= surface.WorldPosition.z + REFRACTION_WATER_PLANE_Z_BIAS;

    float rawRefractionWaterDepth = hasRefractionReceiver
        ? max(refractionReceiverLinearDepth - _IN.SV_Position.w, 0.0)
        : 0.0;
    float refractionWaterDepth = hasRefractionReceiver
        ? min(rawRefractionWaterDepth, WATER_FOG_MAX_OPTICAL_DEPTH_CM)
        : (receiverDepthIsMissing ? WATER_FOG_MAX_OPTICAL_DEPTH_CM : 0.0);

    // Re-evaluate every depth-dependent optical term from the same receiver
    // sampled by the final refracted scene color. Missing floor depth behaves
    // like deep open water; an above-water receiver still gets no water fog.
    bool hasWaterOpticalPath = hasRefractionReceiver || receiverDepthIsMissing;
    sceneIntersectionFade = hasWaterOpticalPath ? saturate(refractionWaterDepth * 0.02) : 0.0;
    float unifiedRayThickness = max(0.001, refractionWaterDepth);
    float unifiedVerticalThickness = hasRefractionReceiver
        ? max(0.001, min(max(surface.WorldPosition.z - refractionReceiverWorldPosition.z, 0.0), WATER_FOG_MAX_OPTICAL_DEPTH_CM))
        : (receiverDepthIsMissing ? WATER_FOG_MAX_OPTICAL_DEPTH_CM : 0.001);

    surfaceTerm70 = saturate(refractionWaterDepth * 0.02) * saturate(surfaceTerm16);
    surfaceTerm73 = exp2(surfaceTerm72 * unifiedRayThickness);
    surfaceTerm77 = exp2(surfaceTerm74 * unifiedRayThickness);
    surfaceTerm78 = exp2(surfaceTerm75 * unifiedRayThickness);
    surfaceTerm79 = exp2(surfaceTerm76 * unifiedRayThickness);
    surfaceTerm81 = exp2(surfaceTerm72 * unifiedVerticalThickness);
    surfaceTerm82 = surfaceTerm74 * unifiedVerticalThickness;
    surfaceTerm83 = surfaceTerm75 * unifiedVerticalThickness;
    surfaceTerm84 = surfaceTerm76 * unifiedVerticalThickness;

    WaterSurfaceEvaluation result;
    result.Surface = surface;
    result.BlueNoise = blueNoiseSample;
    result.SceneIntersectionFade = sceneIntersectionFade;
    result.IgnoreViewLightModifier = surfaceTest4;
    result.NormalViewDot = normalViewDot;
    result.AbsNormalViewDot = surfaceAbsoluteValue;
    result.ReflectionBasis = reflectionBasis;
    result.ClampedNormalViewDot = surfaceTerm19;
    result.MinimumRoughness = surfaceTerm85;
    result.DielectricF0 = dielectricF0;
    result.ParticipatingMediumMask = participatingMediumMask;
    result.ClusterCellIndex = surfaceIndex22;
    result.SurfaceCoverage = surfaceCoverage;
    result.OpenWaterMask = surfaceTransmittanceMask;
    result.ReferenceUnitTransmittance = surfaceTerm0;
    result.UnitTransmittance = float3(surfaceTerm1, greenExtinction, surfaceTerm2);
    result.TransmissionTint = materialSample13.rgb;
    result.TransmissionEnergy = surfaceTerm69;
    result.SurfaceDensityBlend = surfaceTerm70;
    result.ReferencePathTransmittance = surfaceTerm73;
    result.PathTransmittance = float3(surfaceTerm77, surfaceTerm78, surfaceTerm79);
    result.DeepReferenceTransmittance = surfaceTerm81;
    result.DeepExtinctionLog = float3(surfaceTerm82, surfaceTerm83, surfaceTerm84);
    result.AverageFresnelResponse = surfaceTerm91;
    result.MultipleScatteringWeight = multipleScatteringWeight;
    result.TransmissionWeight = transmissionWeight;
    result.ScreenUv = unrefractedUv;
    result.RefractionUv = refractionUv;
    result.RefractionReceiverDeviceDepth = refractionReceiverDeviceDepth;
    result.RefractionReceiverLinearDepth = refractionReceiverLinearDepth;
    result.RefractionReceiverWorldPosition = refractionReceiverWorldPosition;
    result.RefractionWaterDepth = refractionWaterDepth;
    return result;
}
...
float3 SampleRefractedSceneColor(float2 refractedUv, float2 unrefractedUv)
{
#if defined(ENABLE_REFRACTION_CHROMATIC_DISPERSION)
    // Green follows the base ray; red bends farther and blue bends less.
    float2 dispersionOffset = (refractedUv - unrefractedUv) * REFRACTION_DISPERSION_STRENGTH;
    float red = TranslucentBasePass_SceneColorCopyTexture.SampleLevel(View_SharedBilinearClampedSampler, ClampRefractionUvToView(refractedUv + dispersionOffset), 0.0).r;
    float green = TranslucentBasePass_SceneColorCopyTexture.SampleLevel(View_SharedBilinearClampedSampler, ClampRefractionUvToView(refractedUv), 0.0).g;
    float blue = TranslucentBasePass_SceneColorCopyTexture.SampleLevel(View_SharedBilinearClampedSampler, ClampRefractionUvToView(refractedUv - dispersionOffset), 0.0).b;
    return View_OneOverPreExposure * float3(red, green, blue);
#else
    return View_OneOverPreExposure * TranslucentBasePass_SceneColorCopyTexture.SampleLevel(View_SharedBilinearClampedSampler, ClampRefractionUvToView(refractedUv), 0.0).rgb;
#endif
}
...
OutputStruct main(InputStruct _IN)
{
    ...
    float reflectionCaptureWeight =
        material.ReferencePathTransmittance *
        material.TransmissionEnergy *
        (1.0 - (
            material.SurfaceDensityBlend * material.OpenWaterMask +
            material.SurfaceCoverage));
    ...
    float normalAmbient = EvaluateAmbientDice(surface.Normal, _IN.ENVIRONMENT_AMBIENT_DICE_1.xyz, _IN.ENVIRONMENT_AMBIENT_DICE_2.xyz);
    ...
    float transmissionAmbient = EvaluateAmbientDice(surface.TransmissionDirection, _IN.ENVIRONMENT_AMBIENT_DICE_1.xyz, _IN.ENVIRONMENT_AMBIENT_DICE_2.xyz);

    // Beer-Lambert loss becomes the in-scattering weight. Missing floor depth
    // has already been mapped to the finite deep-water optical-depth clamp.
    ...
    transmittedDiffuse *= lerp(0.25, 1.0, saturate(directional.Visibility));
    ...
    //float3 blendedTransmissionColor = lerp(safeTransmissionColor, safeReflectionColor, screenSpaceReflection.a);
    ...
    float3 transmissionContribution = material.TransmissionWeight * material.MultipleScatteringWeight * (blendedTransmissionColor + direct.SecondaryDiffuse);

    float3 accumulatedColor = specularLighting + direct.Primary.Subsurface + diffuseLighting + refractedSceneColor + transmissionContribution;
