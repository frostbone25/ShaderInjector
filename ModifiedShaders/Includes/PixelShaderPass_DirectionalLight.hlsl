...
// Directional-light values are scaled to the shader's existing 0.325
// thickness range instead of using the local-light world-unit values.
#define CONTACT_SHADOWS_MIN_THICKNESS 0.065

// Multiplier for the view-space size of one pixel at the sampled depth.
#define CONTACT_SHADOWS_PIXEL_THICKNESS_SCALE 25.0
	...
	int shadingModelID)
{
    ...
    #if defined(CONTACT_SHADOWS_IMPROVED_THICKNESS)
        // This is receiver self-occlusion rejection, not the local-light
        // exclusion radius. Directional lights retain their full ray length.
        float receiverNoL = saturate(dot(worldNormal, lightDirection));
    ...
    for (int i = 0; i < CONTACT_SHADOWS_SAMPLES; ++i)
    {
        ...
        #if defined(CONTACT_SHADOWS_IMPROVED_THICKNESS)
            // Treat the jittered sample as the center of a finite ray-depth
            // segment instead of an infinitely thin point.
        ...
        #if defined(CONTACT_SHADOWS_IMPROVED_THICKNESS)
            // Advance even when the sample is rejected as the origin pixel so
            // that rejected receiver-adjacent segments are not folded into the
            // next tested interval.
...
FLightingTerms ShadeDefaultLit(FGBufferData gbufferData, FResolvedPixel resolvedPixel, FDeferredLightData light)
{
    ...
    float diffuseScale = 1.0 - gbufferData.CustomData.z;
        ...
        //float3 diffuse = DiffuseEnergyPreservingOrenNayarSimplified(gbufferData.ShadingDiffuseColor * diffuseScale, roughness, resolvedPixel.LightVector, resolvedPixel.CameraVector, lobe.NoL, noVAbs) * diffuseShadow;
	#else //default game
...
PSOutput main(PSInput input)
{
    ...
    float diffuseLuminance = LuminanceRec709(diffuseLighting.rgb);
    ...
    //output.Color = 0.0f;
    //output.Color = saturate(dot(graphicsBuffer.WorldNormal, resolvedPixel.LightVector)) * saturate(dot(-resolvedPixel.CameraVector, resolvedPixel.LightVector));
    //output.Color = saturate(dot(graphicsBuffer.WorldNormal, normalize(-resolvedPixel.CameraVector + resolvedPixel.LightVector)));
